...
class Monsters(commands.Cog):
    ...
    async def monster(self, ctx, monster: str):
        ...
        embed.add_field(name="Quick Stats", value=f"\u200b{text}", inline=True)
        ...
        embed.add_field(name="Ability Score", value=f"\u200b{text}", inline=True)

        # Profs
        text = '\n'.join([f"**{dic['name']}** `+{dic['value']}`" for dic in monster_data['proficiencies']])
        embed.add_field(name="Proficiencies", value=f"\u200b{text}", inline=True)
        ...
        embed.add_field(name="Details", value=f"\u200b{text}", inline=False)
