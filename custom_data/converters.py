...
class MongoDatabase:
    ...
    def add_user_spells(self, user_id: int, name: str, url: str) -> [dict, int]:
        ...
        logging.log(logging.DEBUG, "User with Id: {} returned with results: {}".format(user_id, current_data))
        if current_data is not None:
            ...
            logging.log(logging.INFO, "Data inserted into area with UserId: {},\n"
                                      "      resp: {}\n".format(user_id, {'_id': resp.inserted_id, 'complete': resp.acknowledged}))
            return resp.inserted_id
